...
export const authenticatePartner = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (req.user?.portalType !== "partner") {
    return res.status(403).json({ message: "Acesso permitido apenas para parceiros" });
  }
  
  next();
};

// Middleware para garantir que o usuário seja um gestor de polo
export const authenticatePolo = (req: Request, res: Response, next: NextFunction) => {
  ...
  if (req.user?.portalType !== "polo") {
    return res.status(403).json({ message: "Acesso permitido apenas para gestores de polos" });
  }
