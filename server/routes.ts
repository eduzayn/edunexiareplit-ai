...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // Buscar cursos publicados disponíveis para o aluno
  app.get("/api/student/courses", requireAuth, async (req, res) => {
    try {
      ...
      const publishedCourses = await storage.getCourses(undefined, "published");
      
      // TODO: Adicionar lógica de matrículas e progresso quando implementarmos
      const coursesWithProgress = publishedCourses.map(course => ({
        ...course,
        progress: Math.floor(Math.random() * 100), // Temporário para demonstração
        isEnrolled: true, // Temporário para demonstração
      }));
  ...
  // Buscar um curso específico para o aluno
  app.get("/api/student/courses/:id", requireAuth, async (req, res) => {
    try {
      ...
      // Buscar os detalhes de cada disciplina
      const disciplineDetails = await Promise.all(
        courseDisciplines.map(async (cd) => {
          const discipline = await storage.getDiscipline(cd.disciplineId);
          return {
            ...discipline,
            order: cd.order,
            // TODO: Adicionar progresso do aluno na disciplina
            progress: Math.floor(Math.random() * 100), // Temporário para demonstração
          };
        })
      );
      
      // Adicionar dados do aluno relacionados ao curso
      const courseWithDetails = {
        ...course,
        disciplines: disciplineDetails.sort((a, b) => a.order - b.order),
        // TODO: Adicionar dados reais de progresso
        progress: Math.floor(Math.random() * 100), // Temporário para demonstração
        enrolledAt: new Date().toISOString(), // Temporário para demonstração
      };
